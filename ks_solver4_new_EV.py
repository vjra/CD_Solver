...
# new edgy
# number_of_mesh_elements_edgy = 5200
...
refinement_radius = 0.85
refinement_radius_center = 0.4
# number_of_mesh_elements_edgy = 25000
# number_of_mesh_elements_center = 2800
# refinement_radius = 0.85
# refinement_radius_center = 0.4
# used for 5*10^-5
...
def weak_formulation_EV(rhovold,V,u,v,dt,delta,epsilon,alpha):
    B = CoefficientFunction(((1/delta)*exp(u[0]/delta),-exp(u[0]/delta),exp(u[0]/delta),1),dims=(2,2))
    b = BilinearForm(V)
    b += SymbolicBFI(CoefficientFunction(exp(u[0]/delta))*v[0] + epsilon*u[1]*v[1]+dt*InnerProduct(B*grad(u),grad(v)) \
         + dt*CoefficientFunction((0,u[1]-exp(alpha*u[0]/delta)))*v - rhovold[0] * v[0]-epsilon*rhovold[1]*v[1])
    return b

def trafo_back(u,gfucalc,delta):
    gfucalc.Set((CoefficientFunction((exp(u[0]/delta),u[1]))))
    return gfucalc
...
def initial_data_radial(V,xi,xi2,mass,mass2,midpointr,midpointc):
    uvold = GridFunction(V)
    machineprecision = 2.220446049250313e-16
    uvold.Set(CoefficientFunction((mass/(pi*xi)*exp(-1/xi*(x-midpointr[0])**2-1/xi*(y-midpointr[1])**2)+ 1e-2+machineprecision,0)))
    return uvold

def initial_data_radial_two(V,xi,xi2,mass,mass2,midpointr,midpointc):
    uvold = GridFunction(V)
    machineprecision = 2.220446049250313e-16
    uvold.Set(CoefficientFunction((mass/(pi*xi)*exp(-1/xi*(x-midpointr[0])**2-1/xi*(y-midpointr[1])**2) + mass/(pi*xi)*exp(-1/xi*(x-(0.3+midpointr[0]))**2-1/xi*(y-(midpointr[1]))**2)+ 1e-2+machineprecision,0)))
    return uvold
...
def run_EV(experiment_name,rhovold,gfucalc,gfucalcw,gfuL2,b,mesh,dt,nsteps,delta,paramlisto,startingtimestep,visualoutput_solver):

    initialmass = Integrate(rhovold,mesh)
    with TaskManager():
        for i in range(startingtimestep,nsteps):
            atime = time()
            rhovold.Set(gfucalc)
            SimpleNewtonSolve(gfucalcw,b)
            modulo_constant = 1
            gfucalc = trafo_back(gfucalcw,gfucalc,delta)
