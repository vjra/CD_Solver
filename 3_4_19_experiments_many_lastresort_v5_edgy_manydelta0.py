...
def fill_in_param(expridata):
    ...
    experiment_name = 'Many_Expri_{}_alpha_{}_epsilon_{}_delta_{}'.format(expri_number,alpha,epsilon,delta)
    return  midpointr, radius, midpointc, meshsize, geometry, order, T, dt, xi, xi2, mass1, mass2,mass3, expri_number, alpha, epsilon, delta, experiment_name
...
for i in range(2,3,1):
    tempexpri = {'exprinum': int('9{}'.format(i)),'alpha': 1, 'delta': 0 , 'epsilon': 1, 'dt': 1e-4,'T': 2.5, 'meshsize': 0.035}
...
for experiment in experiment_list:
    midpointr, radius, midpointc, meshsize, geometry, order, T, dt, xi, xi2, mass1, mass2,mass3, expri_number, alpha, epsilon, delta, experiment_name = fill_in_param(experiment)
    try:
    ...
    while counter_attempts < 6:
        try:
            ...
            if cont_switch == True:
                ...
                uvold = initial_data_many(mass1,V,midpointr,xi,mass2,mass3)
