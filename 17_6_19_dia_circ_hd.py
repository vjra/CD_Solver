...
# rect
    ...
    experiment_name = 'DIA_Expri_HD_{}_alpha_{}_epsilon_{}_delta_{}'.format(expri_number,alpha,epsilon,delta)
...
experiment_list = []
# alllist = [10**(-1)/2,10**(-1)/4,10**(-2),10**(-2)/2,10**(-2)/4,10**(-3),10**(-3)/2,10**(-3)/4]
# delta_list = [10**(-1)/2,10**(-1)/4,10**(-2)/4,10**(-3)/4] # done with dt =1e-2 and dt=1e-3 if delta < 10^(-2)/2
delta_list = [10**(-3)/4]
print(delta_list)
j=4
...
for i in delta_list:
    j+=1
    tempexpri = {'exprinum': int('60{}'.format(j)),'alpha': 1, 'delta': i , 'epsilon': 1, 'dt': 1e-4,'T': 5, 'meshsize': 0.02}
...
for experiment in experiment_list:
    ...
    if delta < 10**(-2)/2:
        print('change data because of small delta')
        T=5
        dt = 1e-4
    ...
    while counter_attempts < 1:
        try:
            ...
            if cont_switch == True:
                ...
                uvold =  initial_data_radial(V,xi,xi2,mass1,mass2,midpointr,midpointc)
